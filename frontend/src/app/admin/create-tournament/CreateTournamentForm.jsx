...
const DivisionCard = ({ division, onUpdate, onRemove }) => {
  ...
  const handleAddWeights = () => {
    const weightsToAdd = newWeights.split(/\s+|,/g)
      .map(w => w.trim())
      .filter(w => w.length > 0 && !division.weights.includes(w));
    ...
    if (weightsToAdd.length > 0) {
      ...
      onUpdate(division.id, { ...division, weights: sortedWeights });
      setNewWeights(''); 
    }
  };
...
export default function CreateTournamentForm() {
  ...
  const [tournamentData, setTournamentData] = useState({
    ...
    awarding: '',
    financing: '',
    // Fields for PDF signatures
    presidentTitle: '"Спорт клубтары мен ұйымдары қауымдастығы" ЗТБ Президенті',
    presidentName: '',
    secretaryTitle: '"Астана қаласы Дзюдо федерациясының бас хатшысы',
    secretaryName: '',
  });
  ...
  return (
    <form onSubmit={handleCreateTournament} className="p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <FormSection title="Положение: Бағдарлама және ережелер">
        ...
        <FormSection title="Положение: Марапаттау">
        ...
        <FormSection title="Положение: Қолтаңбалар (PDF үшін)" className="lg:col-span-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1" htmlFor="presidentTitle">Бекітушінің лауазымы</label>
              <input type="text" id="presidentTitle" value={tournamentData.presidentTitle} onChange={handleInputChange} className="w-full bg-navy-700 p-2 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1" htmlFor="presidentName">Бекітушінің аты-жөні</label>
              <input type="text" id="presidentName" placeholder="Мысалы: Н. Сабитов" value={tournamentData.presidentName} onChange={handleInputChange} className="w-full bg-navy-700 p-2 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1" htmlFor="secretaryTitle">Келісушінің лауазымы</label>
              <input type="text" id="secretaryTitle" value={tournamentData.secretaryTitle} onChange={handleInputChange} className="w-full bg-navy-700 p-2 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1" htmlFor="secretaryName">Келісушінің аты-жөні</label>
              <input type="text" id="secretaryName" placeholder="Мысалы: Ж. Қоныспаев" value={tournamentData.secretaryName} onChange={handleInputChange} className="w-full bg-navy-700 p-2 rounded-lg" />
            </div>
          </div>
        </FormSection>
