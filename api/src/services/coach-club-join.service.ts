...
export async function requestJoinClubAsCoach(coachId: string, clubId: string) {
  ...
  if (!coach || coach.role !== UserRole.COACH) {
    throw new CoachClubJoinRequestError(
      "COACH_NOT_FOUND",
      "Тренер табылмады",
      404,
    );
  ...
  if (!club || club.isBlocked || !club.isActive) {
    throw new CoachClubJoinRequestError(
      "CLUB_NOT_AVAILABLE",
      "Клуб табылмады немесе қолжетімсіз",
      404,
    );
  }
  ...
  if (pending && pending.clubId !== clubId) {
    throw new CoachClubJoinRequestError(
      ...
      "Алдымен ағымдағы өтінімді қайтарыңыз",
      409,
    );
  }
  ...
  if (existing)
    await prisma.coachClubJoinRequest.delete({ where: { id: existing.id } });

  const newRequest = await prisma.coachClubJoinRequest.create({
    data: { coachId, clubId },
    include: {
      club: { select: { id: true, name: true, city: true, logoUrl: true } },
    },
  ...
  sendSystemNotification({
    type: "announcement",
    titleKey: "notification.new_coach_request_title",
    bodyKey: "notification.new_coach_request_body",
    target: { kind: "club", clubId },
    payload: { coachId, coachName: `${coach.name} ${coach.surname}` },
...
export async function cancelCoachJoinRequest(
  coachId: string,
  requestId: string,
) {
  ...
  if (req.status !== "PENDING") {
    throw new CoachClubJoinRequestError(
      "REQUEST_LOCKED",
      "Бұл өтінімді қайтаруға болмайды",
      400,
    );
  }
...
export async function reviewCoachJoinRequest(
  ownerId: string,
  requestId: string,
  approve: boolean,
) {
  ...
  if (req.clubId !== owner.clubId)
    throw new CoachClubJoinRequestError("FORBIDDEN", "Рұқсат жоқ", 403);
  if (req.status !== "PENDING")
    throw new CoachClubJoinRequestError(
  ...
  if (!approve) {
    await prisma.coachClubJoinRequest.update({
      where: { id: requestId },
    ...
    sendSystemNotification({
      type: "announcement",
      titleKey: "notification.join_rejected_title",
      bodyKey: "notification.join_rejected_body",
      target: { kind: "user", userId: req.coachId },
      payload: { clubId: req.clubId, clubName: owner.clubId },
    }).catch((e) => console.error("Failed to send notification:", e));
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.coachClubJoinRequest.update({
      where: { id: requestId },
  ...
  sendSystemNotification({
    type: "announcement",
    titleKey: "notification.join_approved_title",
    bodyKey: "notification.join_approved_body",
    target: { kind: "user", userId: req.coachId },
    payload: { clubId: req.clubId, clubName: owner.clubId },
  }).catch((e) => console.error("Failed to send notification:", e));
...
export async function removeCoachFromClub(
  ...
  if (owner.id === coachId) {
    throw new CoachClubJoinRequestError(
      ...
      "Алдымен клуб иесін ауыстырыңыз",
      400,
    );
  }
...
export async function transferClubOwnership(
  ...
  if (
    !nextOwner ||
    nextOwner.role !== UserRole.COACH ||
    nextOwner.clubId !== clubId
  ) {
    throw new CoachClubJoinRequestError(
      "COACH_NOT_FOUND",
      "Жаңа иесі осы клубтың тренері болуы керек",
      404,
    );
  }
  ...
  await prisma.$transaction([
    ...(owner.role === UserRole.COACH
      ? [
          prisma.user.update({
            where: { id: owner.id },
            data: { clubRole: ClubRole.COACH },
          }),
        ]
      : []),
    prisma.user.update({
      where: { id: coachId },
      data: { clubRole: ClubRole.OWNER },
    }),
    prisma.club.update({
      where: { id: clubId },
      data: { createdById: coachId },
    }),
  ]);
  return { ok: true };
}
...
async function assertClubOwner(userId: string, clubId?: string) {
  ...
  if (user.role === UserRole.ADMIN) {
    if (!clubId)
      throw new CoachClubJoinRequestError("CLUB_REQUIRED", "Клуб қажет", 400);
    return { ...user, clubId };
  }
  if (
    user.role !== UserRole.COACH ||
    !user.clubId ||
    user.clubRole !== ClubRole.OWNER
  ) {
    throw new CoachClubJoinRequestError(
      "FORBIDDEN",
      "Бұл әрекетті тек клуб иесі орындай алады",
      403,
    );
  }
