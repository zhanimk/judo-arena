...
export async function generateBracketPdf(bracketId: string): Promise<Buffer> {
  ...
  if (!bracket)
    throw new PdfError("BRACKET_NOT_FOUND", "Сетка не найдена", 404);
...
export async function generateAllBracketsPdf(
  tournamentId: string,
): Promise<Buffer> {
...
function drawTournamentCover(
  ...
  doc
    ...
    .fontSize(14)
    .fillColor("#FFFFFF")
    .text(`${tournament.location}, ${tournament.city}`, M, 105, {
      width: PW - 2 * M,
      align: "center",
    });
  ...
  doc
    ...
    .fontSize(12)
    .fillColor("#9CA3AF")
    .text(dateRange(tournament.startDate, tournament.endDate, locale), M, 125, {
      width: PW - 2 * M,
      align: "center",
    });
  ...
  const contentsLabel =
    locale === "kk" ? "САНАТТАР" : locale === "en" ? "CATEGORIES" : "КАТЕГОРИИ";
  doc
    .font(fonts.bold)
    .fontSize(11)
    .fillColor("#D4AF37")
  ...
  brackets.forEach((br, i) => {
    ...
    const formatLabel =
      br.format === BracketFormat.ROUND_ROBIN
        ? locale === "kk"
          ? "Дөңгелек"
          : "Круговая"
        : "SE-IJF";
...
export function drawBracketOnePage(
  ...
  const matches = [...(bracket.matches ?? [])].sort((a, b) =>
    pdfMatchSort(a, b, bracket.size ?? 8),
  );
  if (matches.length === 0) {
    const empty =
      locale === "kk"
  ...
  if (!isRR) {
    if ((bracket.size ?? 8) <= 16) {
      ...
      doc
        ...
        .fontSize(6)
        .fillColor("#777")
        .text(`Judo-Arena · ${catLabel}`, M, PH - 12, {
          width: PW - 2 * M,
          align: "right",
          lineBreak: false,
        });
  ...
  const rowH = Math.max(
    13,
    Math.min(24, Math.floor((bottom - top - 16) / rowsPerColumn)),
  );
  ...
  matches.forEach((match, idx) => {
    ...
    doc
      .font(match.winnerId ? fonts.bold : fonts.regular)
      .fontSize(labelFont)
      .fillColor(match.winnerId ? "#D4AF37" : "#6B7280");
...
function pdfDrawSEVisualOnePage(
  ...
  const cardW = Math.floor(
    (poolW - 10 - cardGapX * (quartersRound - 1)) / quartersRound,
  );
  const slotH =
    maxRound0 > 1
  ...
  const roundX = (baseX: number, ri: number) =>
    baseX + 6 + ri * (cardW + cardGapX);
  const cardY = (baseY: number, ri: number, mi: number) =>
    baseY +
    labelH +
    5 +
    mi * slotH * Math.pow(2, ri) +
    (slotH * (Math.pow(2, ri) - 1)) / 2;
...
function pdfDrawSECompactOnePage(
  ...
  const extra = allMatches
    .filter(
      (m) =>
        ...
        m.bracketSection === "bronze1" ||
        m.bracketSection === "bronze2",
    )
  ...
  const roundGap = 14;
  const cardW = Math.floor(
    (PW - 2 * M - champW - roundGap * rounds) / (rounds + 1),
  );
  ...
  const roundLabel = (ri: number) => {
    ...
    if (left === 0)
      return locale === "kk" ? "Финал" : locale === "en" ? "Final" : "Финал";
    if (left === 1)
      return locale === "kk"
        ? "Жартылай финал"
        : locale === "en"
  ...
  for (let ri = 0; ri < rounds; ri++) {
    doc
      ...
      .fontSize(6.5)
      .fillColor("#B77900")
      .text(roundLabel(ri), posX(ri), mainTop, {
        width: cardW,
        align: "center",
        lineBreak: false,
      });
  ...
  const champLabel =
    locale === "kk" ? "Чемпион" : locale === "en" ? "Champion" : "Чемпион";
  ...
  doc
    .roundedRect(champX, champY, champW, cardH, 3)
    .lineWidth(1.1)
    .strokeColor("#D4AF37")
    .stroke();
  doc
    ...
    .fontSize(5.8)
    .fillColor("#B77900")
    .text(champLabel, champX + 4, champY + 4, {
      width: champW - 8,
      align: "center",
      lineBreak: false,
    });
  doc
    .font(fonts.bold)
    .fontSize(cardH <= 28 ? 7 : 8)
    .fillColor("#111827")
    .text(
      champion ? pdfAthleteBracketName(champion) : "TBD",
      champX + 4,
      champY + cardH / 2 - 1,
      {
        width: champW - 8,
  ...
  doc
    .roundedRect(M, extraTop, PW - 2 * M, extraH - 10, 5)
    .lineWidth(0.45)
    .strokeColor("#E5D7A8")
    .stroke();

  const repechage = extra.filter((m) => m.bracketSection === "repechage");
  const bronze = extra.filter(
    (m) => m.bracketSection === "bronze1" || m.bracketSection === "bronze2",
  );
  const repLabel =
    locale === "kk" ? "Жұбату" : locale === "en" ? "Repechage" : "Репашаж";
  const bronzeLabel =
    locale === "kk" ? "Қола үшін" : locale === "en" ? "Bronze" : "За бронзу";
  const sectionGap = 20;
  ...
  const repW = repechage.length
    ? Math.max(
        74,
        Math.floor(
          (secW - 16 - eGap * (repechage.length - 1)) / repechage.length,
        ),
      )
    : 90;
  const bronzeW = bronze.length
    ? Math.max(
        92,
        Math.floor((secW - 16 - eGap * (bronze.length - 1)) / bronze.length),
      )
    : 110;
  ...
  doc
    ...
    .fontSize(7)
    .fillColor("#B77900")
    .text(bronzeLabel, bronzeX + 8, extraTop + 7, {
      width: secW - 16,
      align: "center",
      lineBreak: false,
    });
...
function pdfDrawSEFinalsVisualPage(
  ...
  pdfStrongText(doc, pageTitle, M, 16, {
    width: PW - 2 * M,
    align: "center",
    lineBreak: false,
  });
  ...
  const sfLabel =
    locale === "kk"
      ...
      : locale === "en"
        ...
        : "Полуфиналы";
  const finalLabel =
    locale === "kk" ? "Финал" : locale === "en" ? "Final" : "Финал";
  const championLabel =
    locale === "kk" ? "Чемпион" : locale === "en" ? "Champion" : "Чемпион";
  const repechageLabel =
    locale === "kk" ? "Жұбату" : locale === "en" ? "Repechage" : "Репашаж";
  const bronzeLabel =
    locale === "kk" ? "Қола үшін" : locale === "en" ? "Bronze" : "За бронзу";
  ...
  const semis = finalMatches
    .filter(
      (m) =>
        ...
        (m.round ?? 0) === semisRound,
    )
    .sort((a, b) => (a.position ?? 0) - (b.position ?? 0))
    .slice(0, 2);
  const finalMatch =
    finalMatches.find(
      (m) => m.bracketSection === "final" && (m.round ?? 0) === totalRounds,
  ...
  doc
    ...
    .fontSize(8.5)
    .fillColor("#B77900")
    .text(championLabel, champX, top - 8, {
      width: cardW,
      align: "center",
      lineBreak: false,
    });
  ...
  if (semis[0])
    pdfDrawTinyCard(doc, fonts, semis[0], sfX, sfY1, cardW, cardH, locale);
  if (semis[1])
    pdfDrawTinyCard(doc, fonts, semis[1], sfX, sfY2, cardW, cardH, locale);
  if (finalMatch)
    pdfDrawTinyCard(
      doc,
      fonts,
  ...
  if (finalMatch) {
    doc
      .moveTo(finalX + cardW, finalY + cardH / 2)
      .lineTo(champX, champY + cardH / 2)
      .stroke();
  }
  doc.restore();
  ...
  doc
    .roundedRect(champX, champY, cardW, cardH, 3)
    .lineWidth(1.2)
    .strokeColor("#D4AF37")
    .stroke();
  doc
  ...
  doc
    .roundedRect(M, lowerTop - 18, PW - 2 * M, PH - lowerTop - 22, 5)
    .fill("#FFFFFF");
  doc
    .roundedRect(M, lowerTop - 18, PW - 2 * M, PH - lowerTop - 22, 5)
    .lineWidth(0.45)
  ...
  const bronze = finalMatches
    ...
    .sort((a, b) =>
      (a.bracketSection || "").localeCompare(b.bracketSection || ""),
    );
  ...
  const repCardW =
    repechage.length > 0
      ? Math.min(
          170,
          Math.floor(
            (secW - smallGap * (repechage.length - 1)) / repechage.length,
          ),
        )
      : 170;
  const bronzeCardW =
    bronze.length > 0
      ? Math.min(
          185,
          Math.floor((secW - smallGap * (bronze.length - 1)) / bronze.length),
        )
      : 185;
  ...
  doc
    ...
    .fontSize(10)
    .fillColor("#B77900")
    .text(repechageLabel, M + 10, lowerTop - 8, {
      width: secW - 20,
      align: "center",
      lineBreak: false,
    });
  ...
  doc
    ...
    .fontSize(10)
    .fillColor("#B77900")
    .text(bronzeLabel, bronzeX + 10, lowerTop - 8, {
      width: secW - 20,
      align: "center",
      lineBreak: false,
    });
  bronze.forEach((m, i) => {
    pdfDrawTinyCard(
      ...
      fonts,
      m,
      bronzeX + 10 + i * (bronzeCardW + smallGap),
      lowerTop + 14,
      bronzeCardW,
      smallH,
      locale,
  ...
  doc
    ...
    .fontSize(7)
    .fillColor("#777")
    .text(`Judo-Arena · ${catLabel}`, M, PH - 14, {
      width: PW - 2 * M,
      align: "right",
      lineBreak: false,
    });
...
function _drawBracketPage(
  ...
  doc
    ...
    .fontSize(7.5)
    .fillColor("#9CA3AF")
    .text(`${locStr}   ·   ${dateStr}   ·   ${formatLabel}`, M, 43, {
      width: PW - 2 * M,
      align: "center",
      lineBreak: false,
    });
...
function pdfMatchStageLabel(
  ...
  if (section === "main") {
    const pool =
      "ABCD"[
        matchPoolIdx(match.position ?? 0, match.round ?? 1, bracketSize)
      ] ?? "?";
    const stage =
      (match.round ?? 1) <= quartersRound
  ...
  if (section === "repechage")
    return locale === "kk"
      ? "Жұбату"
      : locale === "en"
        ? "Repechage"
        : "Репашаж";
  if (section === "bronze1" || section === "bronze2")
    return locale === "kk" ? "Қола" : locale === "en" ? "Bronze" : "Бронза";
  return section;
}
...
function pdfMatchResultLabel(match: any, locale: Locale): string {
  ...
  const winner =
    match.redAthlete?.id === match.winnerId
      ? match.redAthlete
      : match.blueAthlete;
  const winnerScore =
    match.redAthlete?.id === match.winnerId
...
function pdfDrawSE(
  ...
  const maxSlot =
    maxRound0 > 1 ? Math.floor((availH - CARD_H) / (maxRound0 - 1)) : 80;
  const SLOT_H = Math.min(
    80,
    Math.max(minSlot, Math.min(maxSlot, minSlot + 4)),
  );

  const halfW = Math.floor((PW - 2 * M - POOL_SEP) / 2);
  const CARD_W = Math.floor(
    (halfW - (quartersRound - 1) * RND_GAP) / quartersRound,
  );
  ...
  const cardY = (ri: number, mi: number) =>
    POOL_TOP +
    mi * SLOT_H * Math.pow(2, ri) +
    (SLOT_H * (Math.pow(2, ri) - 1)) / 2;
  ...
  const drawPoolPair = (poolA: number, poolB: number) => {
    ...
    doc
      .save()
      .lineWidth(0.5)
      .strokeColor("#D4AF3730")
      .moveTo(divX, BAND_H + 2)
      .lineTo(divX, PH - 15)
      .stroke()
      .restore();
    ...
    pools.forEach((poolIdx, pairPos) => {
      ...
      doc
        ...
        .fontSize(9)
        .fillColor("#D4AF37")
        .text(`Pool ${poolLabel}`, base, BAND_H + 6, {
          width: halfW,
          align: "center",
          lineBreak: false,
        });
      ...
      for (let ri = 0; ri < quartersRound; ri++) {
        ...
        doc
          ...
          .fontSize(6.5)
          .fillColor("#6B7280")
          .text(lbl, roundX(base, ri), BAND_H + 18, {
            width: CARD_W,
            align: "center",
            lineBreak: false,
          });
  ...
  doc
    ...
    .fontSize(7)
    .fillColor("#777")
    .text(`Judo-Arena · Pool A–B`, M, PH - 12, {
      width: PW - 2 * M,
      align: "right",
      lineBreak: false,
    });
  ...
  if (hasCDPools && bracket) {
    ...
    doc
      ...
      .fontSize(7)
      .fillColor("#777")
      .text(`Judo-Arena · Pool C–D`, M, PH - 12, {
        width: PW - 2 * M,
        align: "right",
        lineBreak: false,
      });
  ...
  if (bracket) {
    ...
    const catWeight2 =
      bracket.category?.weightMax >= 200
        ...
        : `-${bracket.category?.weightMax} кг`;
    const catLbl2 =
      localize(bracket.category?.name, locale) ||
      `${catGender2} · ${catWeight2}`;
    const finalsLabel =
      locale === "kk"
        ? `${catLbl2} · Финалдар`
        : locale === "en"
...
function pdfDrawFinals(
  ...
  const repLabel =
    locale === "kk" ? "Жұбату" : locale === "en" ? "Repechage" : "Репашаж";
  const brnLabel =
    locale === "kk" ? "Қола" : locale === "en" ? "Bronze" : "Бронза";
  const champLabel =
    locale === "kk" ? "Чемпион" : locale === "en" ? "Champion" : "Чемпион";
  ...
  if (sfs.length >= 2) {
    ...
    doc
      .moveTo(SF_CX, SF1_Y)
      .lineTo(midX, SF1_Y)
      .lineTo(midX, FIN_Y + CARD_H / 2)
      .lineTo(FIN_X, FIN_Y + CARD_H / 2)
      .stroke();
    doc
      .moveTo(SF_CX, SF2_Y)
      .lineTo(midX, SF2_Y)
    ...
    if (finalMatch) {
      ...
      if (CX + CW < PW - M) {
        doc
          ...
          .lineWidth(0.6)
          .strokeColor("#D4AF37")
          .moveTo(FIN_X + CARD_W, CY + CARD_H / 2)
          .lineTo(CX, CY + CARD_H / 2)
          .stroke()
          .restore();

        doc.rect(CX, CY, CW, CARD_H).fill("#FEF3C7");
        doc
          .rect(CX, CY, CW, CARD_H)
          .lineWidth(1.5)
          .strokeColor("#D4AF37")
          .stroke();
        doc
        ...
        if (champion) {
          doc
            ...
            .fontSize(8)
            .fillColor("#0B1426")
            .text(pdfAthleteFullDisplayName(champion), CX + 3, CY + 16, {
              width: CW - 6,
              align: "center",
              lineBreak: false,
            });
          if (champion.club) {
            doc
              ...
              .fontSize(5.5)
              .fillColor("#666")
              .text(localize(champion.club.name, locale), CX + 3, CY + 27, {
                width: CW - 6,
                align: "center",
                lineBreak: false,
              });
  ...
  const REP_CW = Math.min(
    CARD_W,
    Math.floor((PW / 2 - 2 * M) / Math.max(1, repechage.length)) - 6,
  );
  ...
  const BRN_CW = Math.min(
    CARD_W + 20,
    Math.floor((PW / 2 - M) / Math.max(1, bronze.length)) - 6,
  );
  if (bronze.length > 0) {
    doc
      .font(fonts.bold)
  ...
  doc
    ...
    .fontSize(7)
    .fillColor("#777")
    .text(`Judo-Arena · ${new Date().toLocaleDateString()}`, M, PH - 12, {
      width: PW - 2 * M,
      align: "right",
      lineBreak: false,
    });
...
function pdfDrawFlatSE(
  ...
  const CARD_W = Math.min(
    160,
    Math.floor((PW - 2 * M - NUM_ROUNDS * ROUND_GAP) / (NUM_ROUNDS + 0.5)),
  );
  ...
  // Round labels
  const rlabels = ["1/64", "1/32", "1/16", "1/8", "1/4", "1/2", "Финал"].slice(
    -NUM_ROUNDS,
  );
  ...
  for (let ri = 0; ri < NUM_ROUNDS - 1; ri++) {
    for (let mi = 0; mi < mainRounds[ri].length; mi++) {
      const x1 = posX(ri) + CARD_W,
        y1 = posY(ri, mi) + CARD_H / 2;
      const x2 = posX(ri + 1),
        y2 = posY(ri + 1, Math.floor(mi / 2)) + CARD_H / 2;
  ...
  if (extra.length > 0) {
    ...
    if (secY + CARD_H + 20 < PH - 18) {
      ...
      const EW = Math.min(
        145,
        Math.floor((PW - 2 * M - (extra.length - 1) * 10) / extra.length),
      );
...
function pdfDrawRR(
  ...
  const cols = Math.max(
    1,
    Math.floor((PW - 2 * M + COL_GAP) / (CARD_W + COL_GAP)),
  );
  let curY = headerH + M;

  for (const [round, ms] of Array.from(byRound.entries()).sort(
    (a, b) => a[0] - b[0],
  )) {
    const roundLabel =
      locale === "kk"
...
function pdfDrawCard(
  ...
  doc
    .font(redWon ? fonts.bold : fonts.regular)
    .fontSize(nameFont)
    .fillColor(redLost ? "#BBBBBB" : redWon ? "#92660A" : "#1A1A1A");
  pdfTextOneLine(doc, rName, x + 8, y + nameOffsetY, NW);
  if (!compact && red?.club) {
    doc
      .font(fonts.regular)
      .fontSize(clubFont)
      .fillColor(redLost ? "#DDDDDD" : "#AAAAAA");
    pdfTextOneLine(
      doc,
      localize(red.club.name, locale),
      x + 8,
      y + clubOffsetY,
      NW,
    );
  ...
  if (scoreRed) {
    doc
      .font(fonts.regular)
      .fontSize(scoreFont)
      .fillColor(redWon ? "#D4AF37" : "#999999")
      .text(scoreRed, x + w - SW - 2, y + nameOffsetY, {
        width: SW,
        align: "right",
        lineBreak: false,
      });
  ...
  doc
    .font(blueWon ? fonts.bold : fonts.regular)
    .fontSize(nameFont)
    .fillColor(blueLost ? "#BBBBBB" : blueWon ? "#92660A" : "#1A1A1A");
  pdfTextOneLine(doc, bName, x + 8, y + ROW_H + blueNameOffsetY, NW);
  if (!compact && blue?.club) {
    doc
      .font(fonts.regular)
      .fontSize(clubFont)
      .fillColor(blueLost ? "#DDDDDD" : "#AAAAAA");
    pdfTextOneLine(
      doc,
      localize(blue.club.name, locale),
      x + 8,
      y + ROW_H + blueClubOffsetY,
      NW,
    );
  }
  if (scoreBlue) {
    doc
      .font(fonts.regular)
      .fontSize(scoreFont)
      .fillColor(blueWon ? "#D4AF37" : "#999999")
      .text(scoreBlue, x + w - SW - 2, y + ROW_H + blueNameOffsetY, {
        width: SW,
        align: "right",
        lineBreak: false,
      });
...
function pdfDrawTinyCard(
  ...
  doc
    ...
    .lineWidth(done ? 0.8 : 0.45)
    .strokeColor(done ? "#D4AF37" : "#D8D0BE")
    .stroke();
  ...
  doc
    .font(redWon ? fonts.bold : fonts.regular)
    .fontSize(nameFs)
    .fillColor(redLost ? "#A7A7A7" : "#111827");
  pdfTextOneLine(doc, redName, x + 4, y + rowPadY, nameW);

  doc
    .font(blueWon ? fonts.bold : fonts.regular)
    .fontSize(nameFs)
    .fillColor(blueLost ? "#A7A7A7" : "#111827");
...
function pdfEllipsize(
  ...
  while (lo < hi) {
    ...
    if (doc.widthOfString(normalized.slice(0, mid) + ellipsis) <= width)
      lo = mid;
    else hi = mid - 1;
  }
