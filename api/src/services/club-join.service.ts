...
export async function requestJoinClub(athleteId: string, clubId: string) {
  ...
  if (!athlete) throw new JoinRequestError("NOT_FOUND", "Спортшы табылмады", 404);
  if (athlete.clubId) throw new JoinRequestError("ALREADY_IN_CLUB", "Сіз басқа клубта тіркелгенсіз", 409);

  const club = await prisma.club.findUnique({ where: { id: clubId } });
  if (!club) throw new JoinRequestError("CLUB_NOT_FOUND", "Клуб табылмады", 404);
  ...
  if (pending && pending.clubId !== clubId) {
    throw new JoinRequestError("ALREADY_PENDING", "Бір уақытта тек бір клубқа өтінім жіберуге болады", 409);
  }
  ...
  if (existing) {
    if (existing.status === "PENDING") throw new JoinRequestError("ALREADY_PENDING", "Өтінім жіберілді, күтіңіз", 409);
...
export async function listPendingRequests(coachId: string) {
  ...
  if (!coach?.clubId) throw new JoinRequestError("NO_CLUB", "Клубыңыз жоқ", 403);

  return prisma.clubJoinRequest.findMany({
    where: { clubId: coach.clubId, status: "PENDING" },
    orderBy: { createdAt: "asc" },
    include: {
      athlete: { select: { id: true, name: true, surname: true, email: true, gender: true, weightKg: true, dateOfBirth: true } },
    },
  });
}

/** Тренер одобряет или отклоняет заявку */
export async function reviewJoinRequest(coachId: string, requestId: string, approve: boolean) {
  const coach = await prisma.user.findUnique({ where: { id: coachId } });
  if (!coach?.clubId) throw new JoinRequestError("NO_CLUB", "Клубыңыз жоқ", 403);
  ...
  if (req.clubId !== coach.clubId) throw new JoinRequestError("FORBIDDEN", "Рұқсат жоқ", 403);
  if (req.status !== "PENDING") throw new JoinRequestError("ALREADY_REVIEWED", "Өтінім қарастырылды", 409);
  ...
  if (approve) {
    await prisma.$transaction(async (tx) => {
      ...
      if (!athlete) throw new JoinRequestError("NOT_FOUND", "Спортшы табылмады", 404);
      if (athlete.clubId) {
        throw new JoinRequestError("ALREADY_IN_CLUB", "Спортшы басқа клубқа тіркелген", 409);
      }

      await tx.user.update({ where: { id: req.athleteId }, data: { clubId: req.clubId } });
      await tx.clubJoinRequest.update({ where: { id: requestId }, data: { status: "APPROVED" } });
    ...
    await prisma.clubJoinRequest.update({ where: { id: requestId }, data: { status: "REJECTED" } });
  }
...
export async function cancelJoinRequest(athleteId: string, requestId: string) {
  ...
  if (!req || req.athleteId !== athleteId) throw new JoinRequestError("NOT_FOUND", "Өтінім табылмады", 404);
  if (req.status !== "PENDING") throw new JoinRequestError("NOT_PENDING", "Өтінімді тоқтата алмайсыз", 409);
