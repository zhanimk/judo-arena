...
import { CreditCard, Loader2, Scale, Save } from "lucide-react";
...
export function TournamentWeighInTab({ tournamentId }: { tournamentId: string }) {
  ...
  const [clubStatusView, setClubStatusView] = useState<
    "active" | "accepted" | "problem" | "payment" | "all"
  >("active");
  ...
  const markPaid = useMutation({
    mutationFn: (applicationId: string) => api.applications.markPaid(applicationId),
    onSuccess: () => {
      setError("");
      qc.invalidateQueries({ queryKey: ["admin-weigh-in", tournamentId] });
  ...
  const isPaymentCleared = (app: any) =>
    Number(app.paymentAmountKzt ?? 0) <= 0 ||
    app.paymentStatus === "PAID" ||
    app.paymentStatus === "NOT_REQUIRED";
  const paymentBlockedApplications = applications.filter((app: any) => !isPaymentCleared(app));
  const pendingEntryIds = applications.flatMap((app: any) =>
    isPaymentCleared(app)
      ? (app.entries ?? [])
  ...
  const clubStatus = (app: any) => {
    ...
    if (!isPaymentCleared(app)) return "payment";
    const hasProblem = appEntries.some(
      (entry: any) =>
    ...
    const allPassed =
      appEntries.length > 0 && appEntries.every((entry: any) => entry.weighInStatus === "PASSED");
  ...
  const paymentApplications = applications.filter((app: any) => clubStatus(app) === "payment");
  const visibleApplications =
    clubStatusView === "accepted"
      ? acceptedApplications
      : clubStatusView === "problem"
        ? problemApplications
        : clubStatusView === "payment"
          ? paymentApplications
          : clubStatusView === "all"
            ? applications
            : activeApplications;
    ...
    <Panel title={t("weigh_in.panel_title")}>
      ...
      <div className="mb-4 grid gap-3 lg:grid-cols-[1fr_auto]">
        <div className="grid gap-3 sm:grid-cols-5">
          ...
          <ApplicationMetric
            label={t("weigh_in.metric_failed")}
            value={failed}
            tone={failed ? "red" : undefined}
          />
          <ApplicationMetric
            label={t("weigh_in.metric_payment_waiting")}
            value={paymentBlockedApplications.length}
            tone={paymentBlockedApplications.length ? "red" : undefined}
          />
        </div>
        <button
          type="button"
        ...
        <div className="mb-4 grid gap-2 md:grid-cols-4">
          {clubStatusTabs.map((item) => (
            ...
            >
              ...
              <span className="rounded-full bg-muted/60 px-2.5 py-1 text-sm text-foreground">
                {item.count}
              </span>
            </button>
          ))}
      ...
      {applications.length === 0 ? (
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          ...
          {visibleApplications.map((app: any) => {
            const appEntries = app.entries ?? [];
            const paymentCleared = isPaymentCleared(app);
            const clubPendingIds = appEntries
              .filter((entry: any) => paymentCleared && entry.weighInStatus === "PENDING")
              .map((entry: any) => entry.id);
            const passedEntries = appEntries.filter(
              (entry: any) => entry.weighInStatus === "PASSED",
            );
            const waitingEntries = appEntries.filter(
              (entry: any) => entry.weighInStatus === "PENDING" || entry.weighInStatus === "ABSENT",
            );
            const failedEntries = appEntries.filter(
              (entry: any) =>
                entry.weighInStatus !== "PENDING" &&
                entry.weighInStatus !== "ABSENT" &&
                entry.weighInStatus !== "PASSED",
            ...
            const selectedView =
              clubView[app.id] ??
              (clubPendingIds.length > 0 ? "waiting" : clubProblems > 0 ? "failed" : "passed");
            ...
            const viewTabs = [
              ...
              {
                id: "passed" as const,
                label: t("weigh_in.view_passed"),
                count: passedEntries.length,
              },
              {
                id: "waiting" as const,
                label: t("weigh_in.view_waiting"),
                count: waitingEntries.length,
              },
              {
                id: "failed" as const,
                label: t("weigh_in.view_failed"),
                ...
                <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="font-display text-lg font-semibold">
                      {localizeName(app.club?.name)}
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      {app.club?.city} · {appEntries.length}{" "}
                      {t("tournament.athletes").toLowerCase()} · {clubPassed}{" "}
                      {t("weigh_in.metric_passed").toLowerCase()} · {clubPendingIds.length}{" "}
                      {t("weigh_in.metric_pending").toLowerCase()}
                      {clubProblems > 0
                        ? ` · ${clubProblems} ${t("weigh_in.tab_problem").toLowerCase()}`
                        : ""}
                    </div>
                    <div
                    ...
                    >
                      {paymentCleared
                        ? t("weigh_in.payment_cleared")
                        : t("weigh_in.payment_required", {
                            amount: Number(app.paymentAmountKzt ?? 0).toLocaleString("kk-KZ"),
                          })}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {!paymentCleared && Number(app.paymentAmountKzt ?? 0) > 0 && (
                      <button
                        type="button"
                        disabled={markPaid.isPending}
                        onClick={() => markPaid.mutate(app.id)}
                        className="inline-flex items-center gap-2 rounded-md bg-amber-500/15 px-4 py-2 text-sm font-medium text-amber-200 hover:bg-amber-500/20 disabled:opacity-50"
                      >
                        {markPaid.isPending ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ) : (
                    ...
                    <button
                      type="button"
                      disabled={
                        bulkPass.isPending || !paymentCleared || clubPendingIds.length === 0
                      }
                      onClick={() => {
                ...
                <div className="mb-3 grid gap-2 sm:grid-cols-4">
                  {viewTabs.map((item) => (
                    ...
                    >
                      ...
                      <span className="rounded-full bg-muted/60 px-2 py-0.5 text-xs text-foreground">
                        {item.count}
                      </span>
                    </button>
                  ))}
                ...
                <div className="space-y-2">
                  ...
                  {visibleEntries.map((entry: any) => {
                    ...
                    const hasProblemStatus =
                      entry.weighInStatus !== "PENDING" && entry.weighInStatus !== "PASSED";
                      ...
                      >
                        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 text-xs text-muted-foreground">
                              {entry.category?.gender === "MALE"
                                ? t("common.male")
                                : t("tatami.female_short")}{" "}
                              · {entry.category?.ageMin}-{entry.category?.ageMax}{" "}
                              {t("common.years_short")} · {weightLabel(entry.category, t)} ·{" "}
                              {t("weigh_in.app_weight")} {entry.athlete?.weightKg ?? "—"}{" "}
                              {t("common.kg")}
                            </div>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {entry.weighInStatus !== "PASSED" && (
                              <button
                                disabled={update.isPending || !paymentCleared}
                          ...
                          <div className="mt-3 border-t border-border/30 pt-3 space-y-2">
                            ...
                            <div className="flex items-center justify-between">
                              <p className="text-[11px] text-muted-foreground">
                                {t("weigh_in.notes_hint")}
                              </p>
                              <button
                                onClick={() =>
                                  update.mutate({ entryId: entry.id, status: entry.weighInStatus })
                                }
                                disabled={update.isPending}
