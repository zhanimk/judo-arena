...
import { Loader2, AlertTriangle, ArrowRightLeft, CreditCard, Trash2 } from "lucide-react";
...
function ApplicationReviewDetail({
  ...
  const paymentAmount = Number(app.paymentAmountKzt ?? 0);
  const paymentRequired = paymentAmount > 0;
  const paymentCleared =
    !paymentRequired || app.paymentStatus === "PAID" || app.paymentStatus === "NOT_REQUIRED";
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {app.entries?.length ?? 0} {t("common.athletes").toLowerCase()} · {categoryCount}{" "}
            {t("tournament.categories").toLowerCase()} ·{" "}
            {app.submittedAt
              ? new Date(app.submittedAt).toLocaleString()
              : t("applications.not_submitted")}
          </div>
        </div>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="font-medium">{t("payments.status")}</div>
            <div className="mt-1 text-xs opacity-80">
              {t(`payments.status_${app.paymentStatus ?? "NOT_REQUIRED"}`, {
                defaultValue: app.paymentStatus ?? "NOT_REQUIRED",
              })}
              {paymentRequired ? ` · ${paymentAmount.toLocaleString("kk-KZ")} ₸` : ""}
              {app.paymentReference ? ` · ${t("payments.reference")}: ${app.paymentReference}` : ""}
            </div>
          </div>
          {paymentRequired && !paymentCleared && (
            <button
              type="button"
              onClick={() => onMarkPaid(app.id)}
              disabled={isMutating}
              className="inline-flex items-center gap-2 rounded-md border border-amber-400/40 bg-amber-400/15 px-3 py-2 text-xs font-semibold text-amber-100 hover:bg-amber-400/25 disabled:opacity-50"
            >
              {isMutating ? (
                ...
                <div className="space-y-2">
                  {entries.map((entry: any) => {
                    ...
                    return (
                      <div
                        key={entry.id}
                        className={`rounded border px-3 py-2 text-sm ${issues.length ? "border-amber-500/40 bg-amber-500/10" : "border-border/40"}`}
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              {entry.athlete?.gender === "MALE"
                                ? t("common.male")
                                : t("tatami.female_short")}{" "}
                              · {athleteAge(entry.athlete) ?? t("applications.no_age")}{" "}
                              {t("common.years_short")} · {entry.athlete?.weightKg ?? "—"}{" "}
                              {t("common.kg")} · {entry.athlete?.beltRank ?? "—"}
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {moveTargets.length > 0 && (
                              <button
                                onClick={() =>
                                  setShowMoveFor(showMoveFor === entry.id ? null : entry.id)
                                }
                                disabled={isMutating}
...
export function TournamentApplicationsTab({ tournamentId }: { tournamentId: string }) {
  ...
  const adminRemoveEntry = useMutation({
    ...
    onError: (e: any) =>
      setError(e instanceof ApiError ? e.message : t("applications.remove_error")),
  });
  ...
  const markPaid = useMutation({
    ...
    onError: (e: any) =>
      setError(e instanceof ApiError ? e.message : t("payments.mark_paid_error")),
  });
  ...
  const approve = useMutation({
    ...
    onError: (e: any) =>
      setError(e instanceof ApiError ? e.message : t("applications.approve_error")),
  });
  const reject = useMutation({
    ...
    onError: (e: any) =>
      setError(e instanceof ApiError ? e.message : t("applications.reject_error")),
  });
  ...
  const athleteCount = applications.reduce(
    (sum: number, app: any) => sum + (app._count?.entries ?? app.entries?.length ?? 0),
    0,
  );
      ...
      {query.isLoading ? (
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-4">
            <ApplicationMetric
              label={t("applications.metric_club_apps")}
              value={applications.length}
            />
            <ApplicationMetric
              label={t("applications.metric_pending")}
              value={submittedCount}
              tone={submittedCount ? "gold" : undefined}
            />
            <ApplicationMetric
              label={t("applications.metric_approved")}
              value={approvedCount}
              tone="green"
            />
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)]">
            <div className="space-y-2">
              {applications.map((a: any) => {
                ...
                const paymentAmount = Number(a.paymentAmountKzt ?? 0);
                const paymentRequired = paymentAmount > 0;
                const paymentCleared =
                  !paymentRequired ||
                ...
                return (
                  <div
                    key={a.id}
                    className={`rounded-md border p-3 text-sm ${selectedId === a.id ? "border-gold/40 bg-gold/5" : "border-border/60 bg-background/30"}`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <button onClick={() => setSelectedId(a.id)} className="min-w-0 text-left">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {entriesCount} {t("tournament.athletes").toLowerCase()} ·{" "}
                          {a.club?.city ?? "—"}
                        </div>
                      </button>
                      <div className="flex shrink-0 gap-2 items-center">
                        ...
                        {a.status === "SUBMITTED" && (
                          <>
                            <button
                              onClick={() => setReview({ id: a.id, action: "approve", notes: "" })}
                              className="text-xs px-2 py-1 rounded bg-gold/15 text-gold border border-gold/30 hover:bg-gold/25"
                            >
                              ✓
                            </button>
                            <button
                              onClick={() => setReview({ id: a.id, action: "reject", notes: "" })}
                              className="text-xs px-2 py-1 rounded bg-destructive/15 text-destructive border border-destructive/30 hover:bg-destructive/25"
                            >
                              ✕
                            </button>
                          </>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                      ...
                      >
                        {t(`payments.status_${a.paymentStatus ?? "NOT_REQUIRED"}`, {
                          defaultValue: a.paymentStatus ?? "NOT_REQUIRED",
                        })}
                        {paymentRequired ? ` · ${paymentAmount.toLocaleString("kk-KZ")} ₸` : ""}
                      </span>
                      {a.submittedAt && (
                        <span className="rounded-full bg-muted px-2 py-0.5">
                          {t("applications.submitted_short")}{" "}
                          {new Date(a.submittedAt).toLocaleDateString()}
                        </span>
                      )}
                      {a.reviewedAt && (
                        <span className="rounded-full bg-muted px-2 py-0.5">
                          {t("applications.reviewed_short")}{" "}
                    ...
                    {paymentRequired && !paymentCleared && (
                      <button
                        ...
                        className="mt-3 inline-flex items-center gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-xs font-medium text-amber-200 hover:bg-amber-500/20 disabled:opacity-50"
                      >
                        {markPaid.isPending ? (
                          <Loader2 className="h-3.5 w-3.5 animate-spin" />
                        ) : (
            ...
            <div className="rounded-md border border-border/60 bg-background/30 p-4">
              {!selectedId ? (
                <EmptyState
                  title={t("applications.select_hint")}
                  hint={t("applications.select_hint_desc")}
                />
                ...
                <ApplicationReviewDetail
                  ...
                  onMarkPaid={(appId) => markPaid.mutate(appId)}
                  isMutating={
                    adminRemoveEntry.isPending || adminMoveEntry.isPending || markPaid.isPending
                  }
                />
              )}
            </div>
              ...
              <div className="xl:col-span-2 rounded-md border border-gold/30 bg-gold/5 p-4">
                <div className="mb-3 flex items-center gap-2 font-medium">
                  {review.action === "approve"
                    ? t("applications.confirm_approve")
                    : t("applications.confirm_reject")}
                </div>
                <textarea
                  value={review.notes}
                  ...
                  placeholder={
                    review.action === "approve"
                      ? t("applications.notes_placeholder_approve")
                      : t("applications.notes_placeholder_reject")
                  }
                />
