                ...
                {ratingQuery.isLoading ? (
                  ...
                  <ul className="space-y-2 text-sm">
                    {(ratingQuery.data?.entries ?? []).map((e: RatingEntry) => (
                      <li
                        key={e.id}
                        className="flex items-center justify-between glass rounded-md p-3"
                      >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-5">
            <Panel title={t("matches.list_title")}>
              ...
              {matchesQuery.isLoading ? (
                ...
                <EmptyState
                  title={
                    search || statusFilter !== "all" ? t("common.no_data") : t("athlete.no_matches")
                  }
                  hint={
        ...
        <div className="col-span-1 md:col-span-2 overflow-hidden rounded-2xl border border-border/40 bg-card/40 p-6 backdrop-blur-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
              {t("stats.total_matches") ?? "Барлық матчтар"}:{" "}
              <span className="text-foreground text-sm ml-1">{m.total}</span>
            </div>
          ...
          <div className="flex justify-between text-sm font-medium">
            <div className="flex items-center gap-2">
              ...
              <span className="text-emerald-400">
                {m.wins} {t("stats.wins") ?? "Жеңістер"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-rose-400">
                {m.losses} {t("stats.losses") ?? "Жеңілістер"}
              </span>
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <div className="w-24 h-2 bg-black/40 rounded-full overflow-hidden">
                  <div
                    className="bg-white/50 h-full"
                    style={{ width: `${m.wins > 0 ? (m.wazaariWins / m.wins) * 100 : 0}%` }}
                  />
                </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold w-6 text-right tabular-nums text-gold">
                  {m.goldenScoreWins}
                </span>
              </div>
            </div>
