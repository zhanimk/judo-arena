...
function Profile() {
  ...
  const uploadAvatar = useMutation({
    ...
    onError: (e: any) => setError(e instanceof ApiError ? e.message : t("profile.avatar_upload_error")),
  });
  ...
  return (
    <DashboardShell role={t("athlete.role_label")} navItems={nav} accentTitle={t("profile.my_profile")}>
        ...
        >
          ...
          {editing ? (
            ...
            >
              ...
              <Input label={t("profile.name_latin")} value={form.nameLatin} onChange={(v) => setForm({ ...form, nameLatin: v })} />
              <Input label={t("profile.surname_latin")} value={form.surnameLatin} onChange={(v) => setForm({ ...form, surnameLatin: v })} />
              <Input label={t("auth.date_of_birth")} type="date" value={form.dateOfBirth} onChange={(v) => setForm({ ...form, dateOfBirth: v })} />
              <div>
              ...
              <Input label={t("profile.weight_kg")} type="number" step="0.1" min="1" max="300" value={form.weightKg} onChange={(v) => setForm({ ...form, weightKg: v })} />
              <Input label={t("common.belt")} value={form.beltRank} onChange={(v) => setForm({ ...form, beltRank: v })} placeholder={t("profile.belt_placeholder")} />
              <Input label={t("profile.phone")} value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} />
              <div>
                <label className="text-xs uppercase tracking-widest text-muted-foreground">{t("profile.avatar")}</label>
            ...
            <div className="space-y-4">
              ...
              <Field label={t("profile.latin_label")} value={`${user.nameLatin ?? "—"} ${user.surnameLatin ?? ""}`} />
              <Field label={t("common.email")} value={user.email} />
              <Field label={t("auth.date_of_birth")} value={user.dateOfBirth ? new Date(user.dateOfBirth).toLocaleDateString("kk-KZ") : "—"} />
              <Field label={t("common.gender")} value={user.gender === "MALE" ? t("common.male") : user.gender === "FEMALE" ? t("common.female") : "—"} />
              <Field label={t("common.weight")} value={user.weightKg ? `${user.weightKg} ${t("common.kg")}` : "—"} />
        ...
        <div className="space-y-4">
          ...
          <Panel title={t("dashboard.settings")}>
            <div className="space-y-3">
              ...
              <Field label={t("profile.registered_at")} value={new Date(user.createdAt).toLocaleDateString("kk-KZ")} />
              <Field label={t("profile.account_status")} value={user.isActive ? t("common.active") : t("profile.blocked")} />
            </div>
          </Panel>
        </div>
      </div>
...
function ClubJoinSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {pending.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs uppercase tracking-widest text-muted-foreground">{t("profile.sent_requests")}</p>
          {pending.map((r: any) => (
            <div key={r.id} className="flex items-center justify-between gap-2 rounded-lg border border-gold/20 bg-gold/5 p-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{r.club?.city} · {t("profile.pending")}</p>
                </div>
              </div>
              <button
      ...
      {/* Club search */}
      <div>
        <p className="text-xs uppercase tracking-widest text-muted-foreground mb-2">{t("profile.search_club")}</p>
      ...
      {search.length >= 2 && (
        <div className="space-y-1.5">
          {clubsQuery.isLoading && <p className="text-xs text-muted-foreground">{t("common.search")}...</p>}
