...
function AdminScoreboard() {
  ...
  return (
    <DashboardShell
      ...
      navItems={nav}
      accentTitle={t("admin.matches_title")}
    >
...
export function TournamentScoreboardPanel({
  ...
  const [tatamiSessionResult, setTatamiSessionResult] = useState<{
    url: string;
    copied: boolean;
  } | null>(null);
            ...
            <div className="space-y-2">
              {completed.map((m: Match) => {
                ...
                const finishedAt = m.finishedAt
                  ? new Date(m.finishedAt).toLocaleTimeString("kk-KZ", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : "";
                  ...
                  <div key={m.id} className="rounded-lg border border-border/60 bg-card/50 p-3">
                    ...
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                        <span className="rounded bg-muted/60 px-1.5 py-px font-mono">
                          R{m.round}.{m.position}
                        </span>
                        {m.bracket?.category && (
                          <span className="rounded bg-gold/10 px-1.5 py-px text-gold font-medium">
                            {categoryShort(m.bracket.category)}
                          </span>
                        )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        ...
                        <div className="mt-1 font-mono text-xs text-muted-foreground">
                          ...
                          {blueScore.yuko ?? 0} S:{blueScore.shido ?? 0}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  overrideMatch.mutate(
                    {
                      ...
                      winnerSide: overrideDialog.side,
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="mt-1 flex gap-2">
                    <input
                      readOnly
                      value={tatamiSessionResult.url}
                      className="min-w-0 flex-1 rounded-md border border-border bg-input px-3 py-2 font-mono text-xs"
                    />
                    <button
                      onClick={() => {
    ...
    >
      ...
      {!compact && (
        <div className="grid grid-cols-2 gap-2">
          <ScoreSide
            side="RED"
            athlete={match.redAthlete}
            score={match.scoreSnapshot?.red}
            isWinner={isDone && match.winnerId === match.redAthlete?.id}
          />
          <ScoreSide
            side="BLUE"
            athlete={match.blueAthlete}
            score={match.scoreSnapshot?.blue}
            isWinner={isDone && match.winnerId === match.blueAthlete?.id}
          />
        </div>
      )}
