...
function AdminUserDetail() {
  ...
  const [eform, setEform] = useState({
    ...
    email: "", dateOfBirth: "", gender: "" as "" | "MALE" | "FEMALE",
  ...
  const editMut = useMutation({
    mutationFn: () => api.admin.updateUser(id, {
  ...
  const fi = (f: keyof typeof eform) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    ...
    <DashboardShell role={t("admin.role_label")} navItems={nav} accentTitle={`${u.name} ${u.surname}`}>
      ...
      <div className="glass rounded-xl p-4 mb-6 flex items-center gap-3 flex-wrap">
        <span className={`text-[11px] px-2.5 py-1 rounded font-medium ${
          u.role === "ADMIN" ? "bg-gold/15 text-gold" :
        ...
        {u.club && (
          <Link to="/admin/clubs/$id" params={{ id: u.club.id }}
